...
CER 0.0599 — identical to the toolkit-2.2.0 model's baseline at every
concurrency level in `radxa-before.md`/`radxa-concurrency.md`.
...
2.3.2 averages 1156.3 ms across its two runs against 1185.5 ms for 2.2.0 — a
2.5% difference, smaller than the run-to-run spread within either toolkit
version (each run's own min/max spans 350-450 ms) and smaller than the
spread already documented between repeated single-core runs of the same
file in `rk3588-vs-rk3576-asr-2026-09-09.md`. Both files carry the same
...
the RK3576 build's toolkit version) does not produce a measurable latency
improvement over the existing 2.2.0 build, and CER is unchanged (0.0599).
The production model file is unchanged; the 2.3.2 rebuild
(`sha256 56170c2af63c3992587d4bf011b3b85ed0ef26bcec4258aba201a597240ca452`,
490,055,098 bytes) is not published to the model repo.
